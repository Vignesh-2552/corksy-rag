...
- Reorganize `app/` internals into clear layers: `core/` (config, DI, logging), keep `api/`, `services/`, `agents/` (renamed from `workflow/`), `db/`, and split `models/` into `schemas/` (API contracts) and keep domain types where needed
- Add missing `__init__.py` files in all Python packages (including `app/api/v1/` and `app/agents/nodes/`)
...
- **No API behavior changes** — endpoints, services, and agent logic remain functionally identical
